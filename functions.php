    ...
    function leaderboard(){
      require "connection.php";
        ...
        for($i = 0; $i < 30; $i++){
          ...
          $res .= '<tr>
                    <td>#'.strval($i+1).'</td>
                    <td>'.$bel[$i]['name'].' - '.$b.' | '.$bel[$i]['id'].' номер в '.mysqli_fetch_assoc(mysqli_query($conn, "select * from classes where id=".$bel[$i]['class_id'].""))['name'].'</td>
                    <td>'.$maths[$i]['name'].' - '.$m.' | '.$maths[$i]['id'].' номер в '.mysqli_fetch_assoc(mysqli_query($conn, "select * from classes where id=".$maths[$i]['class_id'].""))['name'].'</td>
                    <td>'.$english[$i]['name'].' - '.$e.' | '.$english[$i]['id'].' номер в '.mysqli_fetch_assoc(mysqli_query($conn, "select * from classes where id=".$english[$i]['class_id'].""))['name'].'</td>
                  </tr>';
        }
